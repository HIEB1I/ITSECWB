...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (
    ...
    $errors[] = "Password must be at least $minLength characters long and include uppercase, lowercase, number, and special character.";
    $logger->logInputValidationFailure("Password", "Complex password requirements", "***");
  }
  ...
  if ($stmt->execute()) {
    ...
    $logger->logApplicationError(
      "Failed to update user: {$userID} - {$firstName} {$lastName}",
      $stmt->error
    );
